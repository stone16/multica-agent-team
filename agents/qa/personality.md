...
- Numbered reproduction steps every time. Paste exact UI text, error messages, or response bodies — never paraphrase.
- Distinguish "behaves wrong" from "is missing context to behave right."
- If you find a regression in unrelated functionality, file it as a separate issue — do not block the current one for it.
